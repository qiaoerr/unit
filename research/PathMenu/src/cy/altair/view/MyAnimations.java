...
public class MyAnimations {
	...
	public static void startAnimations(Context context,
			final MenuItemView view, int durationMillis) {
		...
		for (int i = 0; i < view.getChildCount(); i++) {
			...
			float x = view.getFlagX()
					* (float) (radiusByDP * Math.sin(PI / 2
							/ (float) (count - 1) * i));
			float y = view.getFlagY()
					* (float) (radiusByDP * Math.cos(PI / 2
							/ (float) (count - 1) * i));
			...
			} else if (view.getStatus() == MenuItemView.STATUS_OPEN) {// to
																		// close
		...
		int status = view.getStatus() == MenuItemView.STATUS_CLOSE ? MenuItemView.STATUS_OPEN
				: MenuItemView.STATUS_CLOSE;
	...
	// 加号旋转动画
	public static void getRotateAnimation(View view, float fromDegrees,
		...
		if (clockwise) {
			...
			rotate = new RotateAnimation(toDegrees, fromDegrees,
					Animation.RELATIVE_TO_SELF, 0.5f,
					Animation.RELATIVE_TO_SELF, 0.5f);
		}
		rotate.setDuration(durationMillis);
