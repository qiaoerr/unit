...
public class MenuItemView extends ViewGroup {
	...
	public void setPosition(int position) {
		...
		flagX = position == POSITION_LEFT_TOP
				|| position == POSITION_LEFT_BOTTOM ? -1 : 1;
		flagY = position == POSITION_LEFT_TOP || position == POSITION_RIGHT_TOP ? -1
				: 1;
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (positon == 0)
			throw new RuntimeException(
					"PositonUnknow!Use method setPosition to set the position first!");
		if (radius == 0)
			throw new RuntimeException(
		...
		if (changed) {
			...
			for (int i = 0; i < count; i++) {
				...
				int x = (int) (MyAnimations.dip2px(context, radius) * Math
						.sin(MyAnimations.PI / 2 / (float) (count - 1) * i));
				int y = (int) (MyAnimations.dip2px(context, radius) * Math
						.cos(MyAnimations.PI / 2 / (float) (count - 1) * i));
				x = positon == POSITION_RIGHT_BOTTOM
						|| positon == POSITION_RIGHT_TOP ? (getMeasuredWidth()
						- x - width) : x;
				y = positon == POSITION_LEFT_BOTTOM
