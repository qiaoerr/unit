...
public class LeaveMessageAct extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		setContentView(R.layout.layout_liuyan);
	...
	private void initData() {
		...
		util = new TopBarUtil(false, naviStyle, topbar, "客服中心", null , context,
	...
	private void initView() {
		...
		ly_name = (EditText) findViewById(R.id.ly_name);
		ly_tel = (EditText) findViewById(R.id.ly_tel);
		ly_context = (EditText) findViewById(R.id.ly_context);
		ly_email = (EditText) findViewById(R.id.ly_email);
		ly_qq = (EditText) findViewById(R.id.ly_qq);
		ly_sex = (EditText) findViewById(R.id.ly_sex);
	...
	private void getData() {
		...
		if(form != null) {
			if(form.getIsAddressShow() == 1 && form.getIsAddressCheck() == 1) {
				if (StrUtil.isEmpty(ly_add)) {
				 ...
				 if (StrUtil.isNotEmpty(ly_emailStr)&&!ly_emailStr.contains("@")) {
		...
		dialog.show();
		api.createGuessBook(ly_nameStr, gender, memberID, ly_telStr, ly_emailStr, ly_contextStr, ly_add, new RequestListener() {
	...
	private void changeGender() {
		...
		AlertDialog dlg = new AlertDialog.Builder(LeaveMessageAct.this)
				.setTitle("设置")
