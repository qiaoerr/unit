...
public class TaoBaoDiscountActivity extends Fragment implements
		OnItemClickListener {
	...
	private ArrayList<TmallBrand_Cat> catas = new ArrayList<TmallBrand_Cat>();
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				view_loading.close();
				MySimpleAdapter adapter = new MySimpleAdapter(context, catas);
	...
	private void initData() {
		...
		PageConfigParser p = new PageConfigParser(context,
				"layout/TaoBaoDiscountLayout.xml");
		config = p.parser();
	...
	private void getDataFromWeb() {
		...
		String cat = "50100982";
		String method = "tmall.temai.subcats.search";
					...
					public void onComplete(String result) {
						try {
							...
							JSONObject object = new JSONObject(result)
									.optJSONObject(
											"tmall_temai_subcats_search_response")
									.optJSONObject("cat_list");
							JSONArray array = object
									.optJSONArray("tmall_tm_cat");
							for (int i = 0; i < array.length(); i++) {
								TmallBrand_Cat tmallBrand = new TmallBrand_Cat(
										array.getJSONObject(i),
										Constants.TYEP_CAT);
								catas.add(tmallBrand);
							}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		args.putString("cat", catas.get(position).getid() + "");
