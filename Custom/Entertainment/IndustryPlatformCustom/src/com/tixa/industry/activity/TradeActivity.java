...
import com.tixa.industry.adapter.TradeAdapter;
...
public class TradeActivity extends Fragment implements OnItemClickListener {
	...
	private final static String TRADE_LIST= "trade_list.tx";
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			ArrayList<Exhibition> tempData = (ArrayList<Exhibition>) msg.obj;
			if (isDestroy)
				return;
			switch (msg.what) {
			...
			case Data.MOREDATA:
				...
				if (tempData != null && tempData.size() > 0) {
					if (myData == null)
						myData = new ArrayList<Exhibition>();
	...
	private void initView() {
		...
		myData = getFromLocal(TRADE_LIST);
		...
		adapter = new TradeAdapter(context, myData, rowNum);
		
		listView.setAdater(adapter, Constants.CACHE_DIR + appID + "/"+ typeID+ "/"+TRADE_LIST);
	...
	@SuppressWarnings("unchecked")
	private ArrayList<Exhibition> getFromLocal(String fileName) {
		String filePath = Constants.CACHE_DIR + appID + "/"+ typeID
				+ "/"  + fileName;
		ArrayList<Exhibition> data = (ArrayList<Exhibition>) FileUtil.getFile(filePath);
		return data;
	}

	private void saveToLocal(ArrayList<Exhibition> data, String fileName) {
		try {
	...
	private void upData() {
		...
		api.getExhibitionList(rowNum, -1, firstID, new RequestListener() {
			...
			public void onComplete(String response) {
				ArrayList<Exhibition> tempData = new ArrayList<Exhibition>();
				try {
					if (StrUtil.isHttpException(response)) {
						...
						String res = "";
						if (json.has("exhibitionList")) {
							res = json.optString("exhibitionList");
						}
						if (res.equals("none")) {
	...
	private void getHistory() {
		...
		api.getExhibitionList(rowNum, 1, lastID,  new RequestListener() {
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		intent.putExtra("exhibition", myData.get(position));
